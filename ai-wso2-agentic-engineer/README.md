...
#### Generate a task signing key

The BFF signs per-task JWTs with an RSA-2048 key. Generate one if you don't have one:

```bash
openssl genrsa 2048 | openssl pkcs8 -topk8 -nocrypt -out task-signing.pem
```
...
helm install wso2-ae \
  ...
  --set-file wso2-ae-platform.taskSigning.privateKeyPem=./task-signing.pem \
