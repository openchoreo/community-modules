...
- [Cilium](https://cilium.io/) must be installed on the dataplane kubernetes clusters and must be configured as the Container Network Interface (CNI) plugin.
...
1. After above prerequisites are met, configure your Cilium installation in the dataplane kubernetes cluster with following values (using Cilium CLI or Helm Chart) to enable HTTP metrics observability.
...
This module integrates Cilium, OpenChoreo and observability-metrics-prometheus (OpenChoreo community module), and compatible with following versions.
