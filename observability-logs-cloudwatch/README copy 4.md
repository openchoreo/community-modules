...
**For EKS (Pod Identity).** Skip the static-credential flags and let the AWS
SDK default chain pick credentials up from EKS Pod Identity associations.

1. **Create one IAM role** carrying the [Adapter IAM policy](#adapter-iam-policy)
   *plus* the AWS-managed `CloudWatchAgentServerPolicy` (the Agent and Fluent Bit
   need the write path; the adapter needs the query and alerting paths). Use
   this trust policy so EKS Pod Identity can assume it:
...
2. **Create four Pod Identity associations** in the `$NS` namespace, all
   ...
   | `cloudwatch-agent` | CloudWatch Agent DaemonSet (upstream subchart) |
   | `fluent-bit` | Fluent Bit DaemonSet (upstream subchart) |
...
3. **Install the chart with Pod Identity flags** (no static keys, no
   credential-injection hook):
   ...
   helm upgrade --install observability-logs-cloudwatch \
     ...
     --set awsCredentials.name="" \
     --set cloudWatchAgent.injectAwsCredentials.enabled=false \
     --set adapter.alerting.webhookAuth.enabled=true \
     --set adapter.alerting.webhookAuth.sharedSecret="$WEBHOOK_SHARED_SECRET"
   ```

   `awsCredentials.name=""` is required: when non-empty, `deployment.yaml`
   mounts the credentials Secret with `envFrom`, and those env vars take
   precedence over Pod Identity. The `injectAwsCredentials` hook auto-skips
   on an empty name, but disabling it explicitly keeps the install plan
   clean.

   To restore EKS niceties the chart turns off for the k3d / kind path
   (Application Signals entity enrichment, dataplane and host log streams),
   override
   `amazon-cloudwatch-observability.containerLogs.fluentBit.config.extraFiles`
...
### Step 5 — Wire EventBridge to the adapter webhook
