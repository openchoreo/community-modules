...
func TestWebhookAuth_AcceptsBasicAuthPassword(t *testing.T) {
	// GCP webhook_basicauth sends Authorization: Basic base64(user:pass);
	// the adapter matches the secret against the password component.
	mw := newMiddleware("verysecrettokenxx", true)
	h := mw(http.HandlerFunc(passThrough))

	req := httptest.NewRequest(http.MethodPost, "/api/v1alpha1/alerts/webhook", bytes.NewBufferString("{}"))
	req.SetBasicAuth("openchoreo", "verysecrettokenxx")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
...
func TestWebhookAuth_RejectsWrongBasicAuthPassword(t *testing.T) {
	mw := newMiddleware("verysecrettokenxx", true)
	h := mw(http.HandlerFunc(passThrough))

	req := httptest.NewRequest(http.MethodPost, "/api/v1alpha1/alerts/webhook", bytes.NewBufferString("{}"))
	req.SetBasicAuth("openchoreo", "wrongpassword")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
